...
#  for genral settings
@manager.route('/master_create_kb', methods=['post'])
@login_required
@validate_request("name")
def master_create_kb():
    req = request.json
    req["name"] = req["name"].strip()
    req["name"] = duplicate_name(
        KnowledgebaseService.query,
...
#  for law settings
@manager.route('/master_create_update_kb', methods=['post'])
@login_required
@validate_request("name")
def master_create_update_kb():
    req = request.json
    req["name"] = req["name"].strip()
    req["name"] = duplicate_name(
        KnowledgebaseService.query,
    ...
    try:
        ...
        print('eeeee-------->',e)
        print('tttt-------->',t)
        print('embd_iddddddddd-------->',t.embd_id)
        print('embd_iddddddddd-------->',t.llm_id)
        req["embd_id"] = t.embd_id
        if not KnowledgebaseService.save(**req):
            return get_data_error_result()
        update_payload = {
        ...
        try:
            ...
            e, kb = KnowledgebaseService.get_by_id(update_payload["kb_id"])
            print('e1------->', e)
            print('kb1------->', kb)
            ...
            print('check2')
            if update_payload["name"].lower() != kb.name.lower() \
                    and len(KnowledgebaseService.query(name=update_payload["name"], tenant_id=current_user.id, status=StatusEnum.VALID.value)) > 1:
                return get_data_error_result(
                    retmsg="Duplicated knowledgebase name.")
            print('check3')
            del update_payload["kb_id"]
            if not KnowledgebaseService.update_by_id(kb.id, update_payload):
                return get_data_error_result()
            print('check4')
            ...
            print('GEEEET JSSSSON RESULTTTTT', get_json_result(data=kb.to_json()))
